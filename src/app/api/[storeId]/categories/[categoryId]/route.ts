...
export async function GET(
  ...
  try {
    if (!params.categoryId) {
      return new NextResponse("Id kategorije je obvezan", { status: 400 });
    }
    ...
    console.log("[CATEGORY_GET_METHOD]", error);
...
export async function PATCH(
  ...
  try {
    ...
    if (!params.categoryId) {
      return new NextResponse("Id kategorije je obavezan", { status: 400 });
    }
    ...
    console.log("[CATEGORY_PATCH_METHOD]", error);
    return new NextResponse("Internal error", { status: 500 });
  }
}
