...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = (id: string) => {
    ...
    toast.success("ID kategorije je kopiran u međuspremnik");
  };

  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/inventory/${data.id}`);
      router.refresh();
      router.push(`/${params.storeId}/inventory`);
      toast.success("Kategorija uspješno izbrisana");
      router.refresh();
    } catch (error: any) {
      toast.error("Pobrinite se da prvo izbrišete sve proizvode u kategoriji");
    } finally {
      setLoading(false);
      setOpen(false);
    }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/inventory/${data.id}`)
            }
          >
